BEGIN
DELETE trg_bobgoetz_temp;

-- Get the actuals
                 INSERT INTO trg_bobgoetz_temp
...
select z.projectid,
  ...
  from (
       select a.projectid, a.resource_id
         from trg_bobgoetz_temp a
        where a.hours_type in ('Actual','Allocation')
     group by a.projectid, a.resource_id
        minus
       select a.projectid, a.resource_id
         from trg_bobgoetz_temp a
        where a.hours_type = 'ETC'
     group by a.projectid, a.resource_id
       ) z;
...
select z.projectid,
  ...
  from (
       select a.projectid, a.resource_id
         from trg_bobgoetz_temp a
        where a.hours_type in ('ETC','Allocation')
     group by a.projectid, a.resource_id
        minus
       select a.projectid, a.resource_id
         from trg_bobgoetz_temp a
        where a.hours_type = 'Actual'
     group by a.projectid, a.resource_id
       ) z;
...
select z.projectid,
  ...
  from (
       select a.projectid, a.resource_id
         from trg_bobgoetz_temp a
        where a.hours_type in ('Actual','ETC')
     group by a.projectid, a.resource_id
        minus
       select a.projectid, a.resource_id
         from trg_bobgoetz_temp a
        where a.hours_type = 'Allocation'
     group by a.projectid, a.resource_id
       ) z;
