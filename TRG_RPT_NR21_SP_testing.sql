...
IS
    CURSOR projects_cur(p_id niku.nbi_project_current_facts.project_id%TYPE)
    IS
        SELECT DISTINCT b.project_id
          FROM niku.nbi_project_current_facts b
         WHERE SUBSTR(b.project_code,1,2) = 'SR'
           AND b.project_id = NVL (p_id, b.project_id);

    CURSOR phases_cur(p_id niku.nbi_project_current_facts.project_id%TYPE, c_forum VARCHAR2)
    IS
    ...
    CURSOR tasks_cur(p_id niku.nbi_project_current_facts.project_id%TYPE,
    ...
    xOBSUnit           NUMBER;
    x_startdate        niku.nbi_project_res_task_facts.fact_date%TYPE;
    x_enddate          niku.nbi_project_res_task_facts.fact_date%TYPE;
    x_forum            VARCHAR2(25);
    x_SR               VARCHAR2(5);
    xProject           NUMBER;
    current_wbsseq     niku.prtask.prwbssequence%TYPE;
    previous_wbsseq    niku.prtask.prwbssequence%TYPE;
        ...
        INSERT INTO trg_nr21b_temp(
             ...
             SELECT CASE WHEN a.forum = 'FN' -- Finance
               ...
               FROM trg_nr21_temp a,
                    niku.nbi_project_current_facts b,
                    niku.nbi_resource_current_facts c
    ...
    IF x_SR IN ('ALL','OVER') THEN
        ...
        INSERT INTO trg_nr21b_temp(
             ...
             SELECT CASE WHEN SUBSTR(b.project_code,1,2) = 'FN' -- Finance
               ...
               FROM niku.nbi_resource_current_facts c,
                    niku.nbi_project_res_task_facts a,
                    niku.nbi_project_current_facts b
